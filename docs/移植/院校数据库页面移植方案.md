...
<template>
  <div class="filter-card rounded-lg p-6 bg-white/10 backdrop-blur-md">
    ...
    <div class="flex gap-3">
      <n-button @click="resetFilters" secondary>重置</n-button>
      <n-button @click="applyFilters" type="primary">应用筛选</n-button>
    </div>
  </div>
</template>
  ...
  >
    <template #header>
      <div class="flex items-center gap-4">
        ...
        />
        <div>
          <h2 class="text-xl font-bold">{{ school.schoolName }}</h2>
        ...
        <div class="flex gap-2 ml-auto">
          <n-tag v-if="school.is985" type="warning" size="small">985</n-tag>
          <n-tag v-if="school.is211" type="info" size="small">211</n-tag>
          <n-tag v-if="school.isDoubleFirst" type="success" size="small">双一流</n-tag>
        </div>
      </div>
    </template>
    ...
    <template #action>
      <div class="flex gap-3">
        <n-button @click="toggleFavorite" :loading="favoriteLoading">
          <template #icon>
            <i :class="isFavorited ? 'ri-heart-fill text-red-500' : 'ri-heart-line'"></i>
          </template>
          {{ isFavorited ? '取消收藏' : '收藏院校' }}
        </n-button>
        <n-button @click="shareSchool" secondary>
          <template #icon>
            <i class="ri-share-line"></i>
          </template>
          分享
        </n-button>
        <n-button @click="downloadDetail" secondary>
          <template #icon>
