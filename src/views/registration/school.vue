...
import { type SchoolRegistration, registerSchool } from '@/api/talent/registration'
        ...
        <div class="glass-card rounded-lg glow-border p-6 md:p-8 max-w-4xl mx-auto">
          ...
          <div class="flex items-center justify-center mb-8">
            <div class="flex items-center space-x-4">
              <div class="w-8 h-8 rounded-full bg-gradient-to-r from-green-600 to-emerald-600 flex items-center justify-center text-white text-sm font-bold">
                1
              </div>
              <div class="w-12 h-0.5 bg-gray-700" />
              <div class="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-sm">
                2
              </div>
              <div class="w-12 h-0.5 bg-gray-700" />
              <div class="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 text-sm">
          ...
          <div class="flex justify-center mb-8">
            <div class="flex bg-gray-800/50 rounded-lg p-1">
              <button
                class="px-4 py-2 rounded-md text-sm font-medium transition-all" :class="[registrationMode === 'simple' ? 'bg-primary text-white' : 'text-gray-400 hover:text-white']"
                @click="registrationMode = 'simple'"
              >
                极简注册
              </button>
              <button
                class="px-4 py-2 rounded-md text-sm font-medium transition-all" :class="[registrationMode === 'complete' ? 'bg-primary text-white' : 'text-gray-400 hover:text-white']"
          ...
          <!-- 表单内容 -->
          <NForm ref="formRef" :model="formData" :rules="rules" :show-label="false">
            <!-- 院校信息区块 -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <!-- 左侧：基本信息 -->
              <div class="space-y-6">
                <div class="flex items-center mb-4">
                  <i class="ri-school-line ri-lg text-gradient mr-2" />
                ...
                <div v-if="registrationMode === 'complete'">
                  ...
                  <div class="flex items-center space-x-4">
                    <div class="w-16 h-16 rounded-lg bg-gray-700 flex items-center justify-center overflow-hidden company-logo">
                      ...
                      <i v-else class="ri-school-line text-2xl text-gray-400" />
                    </div>
                    <NUpload
                      :max="1"
                      list-type="image"
                ...
                <div v-if="registrationMode === 'complete'">
                  ...
                  <div class="space-y-3">
                    <div class="flex items-center justify-between">
                      ...
                      <NSwitch v-model:value="formData.is985" />
                    </div>
                    <div class="flex items-center justify-between">
                      <span class="text-sm text-gray-300">211院校</span>
                      <NSwitch v-model:value="formData.is211" />
                    </div>
                    <div class="flex items-center justify-between">
                      <span class="text-sm text-gray-300">双一流院校</span>
                      <NSwitch v-model:value="formData.isDoubleFirst" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-6">
                <div class="flex items-center mb-4">
                  <i class="ri-contacts-line ri-lg text-gradient mr-2" />
                  <h3 class="text-lg font-medium text-white">
                    联系信息
                  </h3>
                </div>
            ...
            <div class="flex justify-center space-x-4 pt-6 border-t border-gray-700">
              <NButton class="secondary-button px-8 py-3" @click="goBack">
                返回角色选择
              </NButton>
              <NButton class="secondary-button px-8 py-3" @click="skipForNow">
                稍后完善
              </NButton>
              <NButton
                type="primary"
